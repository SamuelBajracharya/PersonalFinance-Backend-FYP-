...
def predict_next_day(
    ...
    all_accounts_data_for_user_series = category_daily_spend.pivot(
        ...
        columns=list(set(trained_accounts) | {user_id}), fill_value=0
    )  # Union of trained_accounts and current user_id
    ...
    if not all_accounts_data_for_user_series.empty:
        ...
        all_accounts_data_for_user_series = all_accounts_data_for_user_series.reindex(
            full_dates_range_user_series
        ).fillna(0)
        all_accounts_data_for_user_series.index = (
            all_accounts_data_for_user_series.index.date
        )
    else:
    ...
    if not lstm_input_for_scaling.empty:
        ...
        lstm_input_for_scaling = lstm_input_for_scaling.reindex(
            full_dates_range_lstm
        ).fillna(0)
    ...
    if len(lstm_input_for_scaling) < effective_look_back:
        padding_needed = effective_look_back - len(lstm_input_for_scaling)
        ...
        if not lstm_input_for_scaling.empty:
            ...
            if not lstm_input_for_scaling.empty and lstm_input_for_scaling.index.min():
                start_date_for_padding = pd.to_datetime(
                    lstm_input_for_scaling.index.min()
                ) - pd.to_timedelta(np.arange(padding_needed, 0, -1), unit="D")
            else:  # If lstm_input_for_scaling is entirely empty, assume today as reference
                start_date_for_padding = pd.to_datetime(
                    "today"
                ).date() - pd.to_timedelta(np.arange(padding_needed, 0, -1), unit="D")
            ...
            lstm_input_for_scaling = pd.concat(
                [padding_df, lstm_input_for_scaling]
            ).tail(effective_look_back)
            lstm_input_for_scaling.index = lstm_input_for_scaling.index.date
        else:  # If lstm_input_for_scaling is empty, and we still need look_back, fill with all zeros for trained_accounts
            start_date_for_padding = pd.to_datetime("today").date() - pd.to_timedelta(
                np.arange(effective_look_back, 0, -1), unit="D"
            )
            lstm_input_for_scaling = pd.DataFrame(
                0.0, index=start_date_for_padding, columns=list(trained_accounts)
            )
            lstm_input_for_scaling.index = lstm_input_for_scaling.index.date

    else:
        lstm_input_for_scaling = lstm_input_for_scaling.tail(effective_look_back)
    ...
    last_days_scaled = scaler.transform(
        last_days
    )  # This should now have the correct number of features
    ...
    predicted_amount_raw = float(pred_full.mean())

    # Determine next_day_date from the full user data series to ensure correct date progression
    next_day_date = (
        pd.to_datetime(all_accounts_data_for_user_series.index.max())
        + timedelta(days=1)
        if not all_accounts_data_for_user_series.empty
        else pd.to_datetime("today").date() + timedelta(days=1)
    )

    # Build risk features from global category behavior (same target shape as training).
    global_series = (
        all_accounts_data_for_user_series.mean(axis=1)
        if not all_accounts_data_for_user_series.empty
        else pd.Series(dtype=float)
    )

    if len(global_series) < 14:
        rolling_3_mean = (
            float(global_series.tail(3).mean()) if len(global_series) >= 3 else 0.0
        )
        rolling_7_mean = (
            float(global_series.tail(7).mean()) if len(global_series) >= 7 else 0.0
        )
        rolling_7_std = (
            float(global_series.tail(7).std()) if len(global_series) >= 7 else 0.0
        )
        rolling_14_mean = (
            float(global_series.tail(14).mean())
            if len(global_series) >= 14
            else rolling_7_mean
        )
        rolling_14_std = (
            float(global_series.tail(14).std())
            if len(global_series) >= 14
            else rolling_7_std
        )
        lag_1_amount = float(global_series.iloc[-1]) if len(global_series) >= 1 else 0.0
        lag_2_amount = (
            float(global_series.iloc[-2]) if len(global_series) >= 2 else lag_1_amount
        )
    else:
        rolling_3_mean = float(global_series.tail(3).mean())
        rolling_7_mean = float(global_series.tail(7).mean())
        rolling_7_std = float(global_series.tail(7).std())
        rolling_14_mean = float(global_series.tail(14).mean())
        rolling_14_std = float(global_series.tail(14).std())
        lag_1_amount = float(global_series.iloc[-1])
        lag_2_amount = float(global_series.iloc[-2])
