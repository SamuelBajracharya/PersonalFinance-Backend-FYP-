...
def evaluate_rewards(db: Session, user: User):
    ...
    user_savings = user.savings if user.savings is not None else 0 # Handle None for existing users
    user_completed_budgets_count = user.goals_completed if user.goals_completed is not None else 0
    ...
    for reward in all_rewards:
        ...
        if reward.reward_type == RewardType.XP and user_xp >= reward.requirement_value:
            unlocked = True
        elif reward.reward_type == RewardType.BUDGET_GOALS and user_completed_budgets_count >= reward.requirement_value:
            unlocked = True
        elif reward.reward_type == RewardType.SAVINGS and user_savings >= reward.requirement_value:
            unlocked = True
